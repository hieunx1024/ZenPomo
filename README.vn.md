...
ZenPomo là công cụ đếm giờ Pomodoro chạy trên giao diện dòng lệnh (TUI), tích hợp khay hệ thống (System Tray) và daemon chạy ngầm, hỗ trợ **Linux** (Ubuntu, Debian, Fedora, Arch) và **Windows 10/11**.

Ứng dụng được thiết kế tối giản theo phong cách công cụ Unix cổ điển, tiêu tốn ít tài nguyên (<15MB RAM), không phụ thuộc runtime bên ngoài (Single Static Binary, 100% Go thuần, không CGO) và hỗ trợ điều khiển hoàn toàn bằng bàn phím.
...
- **Tích hợp thanh trạng thái / Widget:** Cung cấp lệnh `zenpomo status` hỗ trợ xuất định dạng text hoặc JSON cho **Waybar**, **Polybar**, **Tmux**, hoặc **i3/Sway**.
- **Âm thanh và thông báo Desktop:** Nhúng sẵn file âm thanh offline (zero latency) và gửi thông báo native khi hoàn thành phiên.
...
Các file thực thi dựng sẵn luôn có sẵn tại mục **[Releases](https://github.com/hieunx1024/ZenPomo/releases)**:

### 1. Dành cho Windows 10/11
* **Bộ cài đặt Setup Wizard (.exe)** *(Khuyên dùng)*:  
  Tải file `zenpomo_1.0.0_windows_setup.exe` $\rightarrow$ Nhấp đúp chuột để cài đặt. Trình cài đặt sẽ tự động tạo biểu tượng ngoài **Desktop**, thêm vào **Start Menu**, đăng ký khay hệ thống tự khởi động và hỗ trợ gỡ cài đặt (Uninstall) trong Windows Settings.
